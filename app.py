...
def get_threat_analysis(text, source_type):
    ...
    if os.path.exists("user_reports.json"):
        try:
            with open("user_reports.json", "r") as f:
                for line in f:
                    try:
                        ...
                        if len(report_content) > 10 and (report_content in text_lower or text_lower in report_content):
                            score += 10
                            flags.append("🚨 **COMMUNITY ALERT:** This message matches a scam reported by other users.")
                            break
                    except:
                        continue
        except:
    ...
    try:
        for file in files:
            ...
            # --- IMAGE SUPPORT ---
                ...
                # ENABLE MULTI-LANGUAGE OCR (English + Hindi + Tamil)
                # Note: You must install Tesseract language packs for this to work.
                image = image.convert('L')
                
                # --- MEMORY SAFETY FIX (The Important Part) ---
                # We removed the line that was multiplying size by 3.
                # Instead, we limit the size to max 1500px to prevent crashes.
                ...
                pixels = sum(hist)
                brightness = sum([i * c for i, c in enumerate(hist)]) / pixels
                if brightness < 128:
                ...
                try:
                    # Attempt multi-language scan (requires language packs)
                    text = pytesseract.image_to_string(image, lang='eng+hin+tam', config='--psm 6')
                except:
                    # Fallback to default (English) if packs missing
...
def get_stats():
    ...
    if os.path.exists("user_reports.json"):
        try:
            with open("user_reports.json", "r") as f:
                lines = f.readlines()
                total_scams = len(lines)
                if total_scams > 0:
                    last_entry = json.loads(lines[-1])
                    last_reported = last_entry.get("timestamp", "Unknown")
        except:
            pass
